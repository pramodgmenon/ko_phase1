<?php
	...
	if(isset($_POST["h_contentid"]) && $_POST["h_contentid"] != ""  && $_POST["h_contentinfo"] == md5("CONTENT_INFO") ){
		$int_contnent_id = $_POST["h_contentid"];
		...
		if ($int_contnent_id == -1) {
			...
			if(isset($_POST["chk_publish"]) && $_POST["chk_publish"] == CONTENT_PUBLISH){
				$this->content->publish=CONTENT_PUBLISH;
			}else{
				$this->content->publish=CONTENT_NOT_PUBLISH;
			}
		...
		if ($this->content->get_detail() == true) {
			...
			if($this->content->contenttype_id == CONTENT_TYPE_HTML ){
